...
func (r *Replica) handlePropose(propose *genericsmr.Propose) {
	...
	if !r.isLeader && r.crtBalnum < 0 {
		...
		r.ReplyProposeTS(&genericsmrproto.ProposeReplyTS{FALSE, -1, state.NIL, propose.Timestamp, propose.ClientId}, propose.Reply)
		return
	}
	...
	if _, present := r.committed[propose.CommandId]; present {
		if r.isLeader || ALL_TO_ALL {
			r.ReplyProposeTS(&genericsmrproto.ProposeReplyTS{TRUE, propose.CommandId, state.NIL, propose.Timestamp, propose.ClientId}, propose.Reply)
		}
		return
	} else {
	...
	if conflict, glb, _ = r.learn(false); conflict {
		...
		for _, cid := range glb {
			...
			if prop, present := r.commandReplies[cid]; present {
				r.ReplyProposeTS(&genericsmrproto.ProposeReplyTS{TRUE, cid, state.NIL, prop.Timestamp, prop.ClientId}, prop.Reply)
